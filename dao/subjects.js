...
const findById = (subjectId, clientId, showOnlyIfPublished) => {
    return db.one(sql.subjects.findById, {subjectId, clientId, forceShow: !showOnlyIfPublished})
        .then(subject => subjectService.processSubject(subject))
};

const addToFavorites = (subjectId, clientId) => {
    return db.none('insert into favorites (client_id, subject_id) values (${clientId}, ${subjectId})', {clientId, subjectId})
};

const removeFromFavorites = (subjectId, clientId) => {
    return db.none('delete from favorites where client_id = ${clientId} and subject_id = ${subjectId}', {clientId, subjectId})
};
